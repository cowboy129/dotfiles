...
" PHP syntax: PHPDoc support {{{1

  syn cluster phpClValues add=phpDocComment
  syn cluster phpClInClass add=phpDocComment
  syn region phpDocComment    matchgroup=phpComment start="/\*\*\s*$"  end="\*/" contains=phpDocTags,@Spell fold
  syn match  phpDocTags       contained "@\(param\|argument\|requires\|exception\|throws\|type\|class\|extends\|see\|link\|member\|module\|method\|title\|namespace\|optional\|default\|base\|file\)\>" nextgroup=phpDocParam,phpDocSeeTag skipwhite
  syn match  phpDocTags       contained "@\(beta\|deprecated\|description\|fileoverview\|author\|license\|version\|returns\=\|constructor\|private\|protected\|final\|ignore\|addon\|exec\)\>"
  syn match  phpDocParam      contained "\%(#\|\w\|\.\|:\|\/\)\+"
  syn region phpDocSeeTag     contained matchgroup=phpDocSeeTag start="{" end="}" contains=phpDocTags

" PHP syntax: Doctrine2 annotation support {{{1
  syn cluster phpClValues add=phpDoctrineTags
  syn cluster phpClInClass add=phpDoctrineTags
  syn match phpDoctrineTags contained "@\(Entity\|Id\|MappedSuperclass\|Version\|PrePersist\|PostPersist\|PreRemove\|PostRemove\|PreUpdate\|PostUpdate\|PostLoad\|LoadClassMetadata\|OnFlush\)\>" display containedin=phpComment,phpDocComment,phpDoctrineParam
  syn match phpDoctrineTags contained "@\(Table\|Column\|GeneratedValue\|SequenceGenerator\|OneToOne\|OneToMany\|ManyToOne\|ManyToMany\|JoinColumn\|JoinTable\|InheritanceType\|DiscriminatorColumn\|DiscriminatorMap\|\)\>" display nextgroup=phpDoctrineParam containedin=phpComment,phpDocComment,phpDoctrineParam
  "syn region phpDoctrineParam contained keepend extend contains=@phpClExpressions matchgroup=phpParent start="(" end=")"
  syn region phpDoctrineParam contained keepend extend contains=phpDoctrineTags start="(" end=")"
...
"   hi phpIdentifierSimply guifg=DarkGray ctermfg=Brown
  ...
  HiLink phpDocComment    Comment       " Slateblue
  HiLink phpDocTags       Special
  HiLink phpDocSeeTag     Function
  HiLink phpDocParam      Function

  HiLink phpDoctrineTags  Special
  HiLink phpDoctrineParam Function
